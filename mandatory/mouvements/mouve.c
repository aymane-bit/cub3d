...
void	mouve_player(t_global *global, double move_x, double move_y)
{
	...
	global->player->old_map_s = global->player->x_p / TILE_SIZE;
	map_y = (new_y / TILE_SIZE);
	global->player->old_map_y = global->player->y_p / TILE_SIZE;
	if (map_y < 0 || map_y >= global->data->h_map || map_x < 0
		|| map_x >= global->data->w_map)
		return ;
	if (global->data->map[map_y]
		&& map_x < (int)ft_strlen(global->data->map[map_y])
		&& global->data->map[global->player->old_map_y][map_x] != '1'
		&& global->data->map[map_y][global->player->old_map_s] != '1')
	{
